...
$w=array(35,35,35,20,20,20,20,20,18,15,15,15,15);
	...
	{ //width
		...
		$this->Cell(20,6,$eachResult["cname"],1);
		$this->Cell(20,6,$eachResult["dob"],1);
		$this->Cell(20,6,$eachResult["gender"],1);
		$this->Cell(20,6,$eachResult["insurance"],1);
		$this->Cell(20,6,$eachResult["period"],1);
...
$pdf->AddPage();
	...
	$countmember=mysql_query("select * from fire where users='$users'   ");
...
$copio=$rows['size'];
$copoo=$rows['location'];
$copooo=$rows['property'];
$copoooo=$rows['material'];
...
$pdf->Ln();
	...
	$pdf->Ln();
	$pdf->Write(6, 'Owner of Property : '.$cop.'');
	
	$pdf->Ln();
	$pdf->Write(6, 'Usage of Property: '.$copo.' ');
	
	$pdf->Ln();
	$pdf->Write(6, 'Size of Property : '.$copio.' ');
	
	$pdf->Ln();
$pdf->Write(6, 'Location   : '.$copoo.'');
	
	$pdf->Ln();
	$pdf->Write(6, 'Number of Property   : '.$copooo.'');
	
	$pdf->Ln();
	$pdf->Write(6, 'Material Used to build   : '.$copoooo.'');
